...
CONTAINS
   ...
   SUBROUTINE rst_write( kt )
      ...
      IF ( .NOT. ln_diurnal_only ) THEN
                  ...
                  END IF
                  ! sub-grid scale energy
                  IF ( ln_sgske ) THEN
                     CALL iom_rstput( kt, nitrst, numrow, 'sgske'  , sgs_ke, ldxios = lwxios ) ! SGS KE
                  END IF
      ENDIF
...
! restart)
   ...
   SUBROUTINE rst_read
      ...
      IF ( ln_sgske ) THEN
         CALL iom_get( numror, jpdom_autoglo, 'sgske'     , sgs_ke, ldxios = lrxios )   ! SGS KE
      END IF
      !
