...
CONTAINS
   ...
   SUBROUTINE dia_wri( kt )
      ...
      REAL(wp)::   ze3   !   -      -
      REAL(wp), DIMENSION(jpk)         ::   z1d   ! 1D workspace
      ...
      ENDIF
      !
      IF ( iom_use("sgske") ) THEN
          CALL iom_put( "sgske"     , sgs_ke(:,:,:) )
      END IF
      !
      IF ( iom_use("sgske_zint") ) THEN
         z2d(:,:) = 0._wp
         DO jk = 1, jpkm1
            DO jj = 1, jpj
               DO ji = 1, jpi
                  z2d(ji,jj) = z2d(ji,jj) + e3t_n(ji,jj,jk) * sgs_ke(ji,jj,jk) * tmask(ji,jj,jk)
               END DO
            END DO
         END DO
         CALL iom_put( "sgske_zint", z2d(:,:) )
      END IF
      !
      !IF ( iom_use("sgske_hint") ) THEN	
      !    z1d(:) = 0._wp
      !     DO jk = 1, jpkm1
      !      DO jj = 1, jpj
      !         DO ji = 1, jpi
      !            z1d(jk) = z1d(jk) + e1t(ji,jj) * e2t(ji,jj) * sgs_ke(ji,jj,jk) * tmask(ji,jj,jk)                  
      !         END DO
      !      END DO
      !   END DO
      !   CALL iom_put( "sgske_hint", z1d )
      !END IF
      !
