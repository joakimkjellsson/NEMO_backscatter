MODULE tradmp
   ...
   !
   USE usrdef_nam, ONLY: rn_dy, rn_dTdy0, rn_dTdz0, rn_domszy, ln_usrdmp, rn_dmp
   !
...
CONTAINS
   ...
   SUBROUTINE tra_dmp( kt )
      ...
      IF ( ln_usrdmp ) THEN
         !
         njdmp = 1650._wp / rn_dy 
         z_wdmp = 165._wp 
         !
         ! Force damping to zero unless tem or sal
         DO jn = 1, jpts
      	     IF ( (jn /= jp_tem) .and. (jn /=jp_sal) ) THEN
      	        zts_dta(:,:,:,jn) = tsb(:,:,:,jn)
      	     ENDIF
         END DO
         !
         ! Set up damping terms
         !
         resto(:,:,:) = 0._wp
         DO jk = 1, jpkm1
            DO jj = 1, jpj
               DO ji = 2, jpi
                  resto(ji,jj,jk) = 0._wp !-1e-5          
               END DO
            END DO 
         END DO
         !         
         DO jk = 1, jpkm1
            DO jj = 1, jpj
               DO ji = 1, jpi
                  !PRINT*,'(ABS(gphit(ji,jj)) - rn_domszy*0.5_wp)',ABS(gphit(ji,jj)),rn_domszy*0.5_wp
                  IF ( ABS(rn_domszy*0.5_wp - ABS(gphit(ji,jj))) <= z_wdmp ) THEN                     
                     resto(ji,jj,jk) = rn_dmp !1e-5
                  !ELSE IF ( (ABS(gphit(ji,jj)) - rn_domszy*0.5_wp) <= (z_wdmp + 100) ) THEN
                  !   resto(ji,jj,jk) = -1e-5 * (1._wp - (ABS(gphit(ji,jj) - rn_domszy*0.5_wp) / (z_wdmp + 100)))
                  END IF 
               END DO
            END DO
            DO jj = 1, jpj
               DO ji = 1, jpi
                  zts_dta(ji,jj,jk,jp_tem) = rn_dTdy0 * (gphit(ji,jj)) * 1.e+3 + &
                                             rn_dTdz0 * (gdept_b(ji,jj,jk)) + &
                                             20._wp
                  zts_dta(ji,jj,jk,jp_sal) = 30._wp
               END DO
            END DO
         END DO
         !zts_dta(:,:,:,:) = tsb(:,:,:,:) 
         !resto(:,:,:) = 0._wp
         !PRINT*,'set usr dmp to 0 '
         IF (iom_use('dmp_coef')) CALL iom_put( 'dmp_coef', resto )
      ELSE
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namtra_dmp in configuration namelist' )
      ...
      IF(lwp) THEN                  ! Namelist print
         ...
         WRITE(numout,*) '         User-defined damping           ln_usrdmp = ', ln_usrdmp 
