MODULE usrdef_nam
   ...
   REAL(wp), PUBLIC ::   rn_ppgphi0 =   37.5 ! reference latitude for beta-plane 
   ...
   REAL(wp), PUBLIC ::   rn_dTdy0   = -5.e-6 ! initial meridional temperature gradient
   REAL(wp), PUBLIC ::   rn_dTdz0   = -8.e-3 ! initial vertical temperature gradient
   LOGICAL,  PUBLIC ::   ln_usrdmp  = .false. 
   REAL(wp), PUBLIC ::   rn_dmp     = 1e-4   
   REAL(wp), PUBLIC ::   rn_ampnoise = 0.1   ! amplitude of SSH noise in ini state
...
CONTAINS
   ...
   SUBROUTINE usr_def_nam( cd_cfg, kk_cfg, kpi, kpj, kpk, kperio )
      ...
      NAMELIST/namusr_def/  rn_domszx, rn_domszy, rn_domszz, rn_dx, rn_dy, rn_dz, rn_0xratio, rn_0yratio   &
         ...
         &                 , rn_dTdy0, rn_dTdz0, ln_usrdmp, rn_dmp, rn_ampnoise
      !!----------------------------------------------------------------------
      !
...
902   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namusr_def in configuration namelist' )
      ...
      IF(lwp) THEN
         ...
         WRITE(numout,*) '      Meridional temperature gradient     rn_dTdy0 = ', rn_dTdy0, 'K/m'
         WRITE(numout,*) '    	Vertical temperature gradient	    rn_dTdz0 = ', rn_dTdz0, 'K/m'
         WRITE(numout,*) '   '
