...
CONTAINS
   ...
   SUBROUTINE usr_def_istate( pdept, ptmask, pts, pu, pv, pssh )
      ...
      CASE(6)
         !
         ! Stephan Jurickes channel setup from FESOM
         !
         !
         ! Constant f
         zf0   = 2._wp * omega * SIN( rad * rn_ppgphi0 )
         !
         ! Set up temperature and salinity
         DO jk=1, jpk
            DO jj=1, jpj
               DO ji=1, jpi
                  ! 25C at southern boundary surface
                  ! Then linearly decreasing to the north and down
                  pts(ji,jj,jk,jp_tem) = rn_dTdy0 * (gphit(ji,jj)) * 1.e+3 + &
                                         rn_dTdz0 * (pdept(ji,jj,jk)) + &
                                         20._wp
                  pts(ji,jj,jk,jp_sal) = 30._wp
               END DO
            END DO
         END DO
         !
      ...
      IF (ln_sshnoise) THEN
         ...
         pssh(:,:) = pssh(:,:) + ( rn_ampnoise  * zrandom(:,:) - 0.5_wp*rn_ampnoise )
      END IF
      CALL lbc_lnk( 'usrdef_istate', pssh, 'T',  1. )
